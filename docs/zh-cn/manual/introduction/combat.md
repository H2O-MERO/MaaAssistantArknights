...
- `使用药剂` + `使用源石` 和 `指定次数`、`指定材料` 三个选项为短路开关（或门），即达成三个选项中的任一条件，均会视为任务完成，停止理智作战。
  ...
  - `指定材料` 指定刷多少个指定材料（例如“获取 5 个固源岩后停止”），支持两种计数模式：
    - `刷取数量`：以本次任务中该材料的掉落数量为目标。
    - `目标库存`：参考 [仓库识别](./tools.md#仓库识别) 中保存的库存数据，只补到设定库存为止。需要先通过 [更新数据](./user-data-update.md) 或 [仓库识别](./tools.md#仓库识别) 获取库存。此模式由 UI 实现（Core 仅支持刷取数量模式）。

- `指定材料` 与 `关卡指定` 是两个互相独立的逻辑。`指定材料` 只是以材料个数作为任务完成依据，并不会自动导航到相应关卡。
...
- 在 `任务设置` - `理智作战` - `高级设置` 中启用。跳过判定由 Core 实现，其他集成工具也可通过 `Fight` 任务参数启用。
- 先检查当前游戏日是否有 SideStory 活动；若有，且连续或重叠的后续活动会在本周结束前终止，则跳过当前理智作战任务，以便在活动结束后的任意一天执行剿灭作战。如果 SideStory 活动连续覆盖本周剩余时间直至周日结束，则不会跳过。
